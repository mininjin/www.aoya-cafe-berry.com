<template>
  <div class="py-10 flex flex-wrap">
    <a v-for="(item, i) in list" :key="item.key" :href="item.href" target="_blank" rel="noopener noreferrer" :class="`w-full md:w-1/2 p-5 transition-all duration-1000 transform ${open[i] ? '' : 'translate-y-10 opacity-0'
    }`" :ref="setRefs">
...
import { LINK_IMAGE_PATH } from "@/constants";
